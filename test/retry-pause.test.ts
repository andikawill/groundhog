...
describe('pause', () => {
  it('calls onPause and continues when told to', async () => {
    mock = await startMock({ 'GET /x': () => ({ body: '{}' }) })
    const paused: string[] = []
    const result = await run(
      { name: 'pause', steps: [{ id: 'x', method: 'GET', url: '{{env.API}}/x', pause: true }] },
      mock.url,
      {
        onPause: async (id) => {
          paused.push(id)
    ...
    expect(paused).toEqual(['x'])
    expect(result.steps[0].status).toBe('passed')
  })

  it('skips the step without sending when told to skip', async () => {
    mock = await startMock({ 'GET /x': () => ({ body: '{}' }) })
    const result = await run(
      { name: 'pause skip', steps: [{ id: 'x', method: 'GET', url: '{{env.API}}/x', pause: true }] },
      mock.url,
      { onPause: async () => 'skip' },
    )
  ...
  it('runs straight through when no onPause handler is supplied', async () => {
    mock = await startMock({ 'GET /x': () => ({ body: '{}' }) })
    const result = await run(
      { name: 'no handler', steps: [{ id: 'x', method: 'GET', url: '{{env.API}}/x', pause: true }] },
      mock.url,
    )
    expect(result.steps[0].status).toBe('passed')
  })
